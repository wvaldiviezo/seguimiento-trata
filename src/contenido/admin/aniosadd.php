...
if(isset($_REQUEST["_id_registro_seguimiento"]) && $_REQUEST["_id_registro_seguimiento"]!=NULL){
    $avance=1;
    $rsql_maestro=$_consulta_sistema->query( "SELECT * FROM seguimiento_despacho_entrega.ssd_anios_seguimmiento where ans_codigo='{$_REQUEST['_id_registro_seguimiento']}' limit 1;" );
...
<div title="Ficha General" style="padding:10px; background:rgba(187, 187, 187, 0.0170588)">
    <form  name="frm_insertaranio_seguimiento_despacho" id="frm_insertaranio_seguimiento_despacho" style="width:100%; height:auto; border:1px solid #CCC">
    <table align="center" id="tableFichaGeneral_seguimiento_despacho" width="99%" cellpadding="5" cellspacing="5">
    <tbody>
    <tr>
        <td colspan="6" style="background:#0072C6; color:#fff">INGRESO AÑOS</td>
    </tr>
            ...
            if ( in_array(40, $perfil,true) || in_array(41, $perfil,true)  ) :
                ...
                <script>
                    function actualizarUsuario_seguimientoDespacho(){
                        $.messager.confirm('Confirmación','Está seguro que desea actualizar el año?.',function(r){
                            if (r){
                                $.ajax( "<?php echo DIR_REL ?>class/validador.php")
                                    .done(function(result) {
                                        if(result==1){
                                            xajax_procesar_frm_insertaranio_seguimiento_despacho(xajax.getFormValues('frm_insertaranio_seguimiento_despacho', true));
                                        }else if(result==0){
                                            location.reload();
                                        }
                                    });
    ...
    <tr>
        ...
        <td width="27%">
            <?php
            if($avance==1){
                echo '<input id="campo_ingreso_seguimientoDespacho_1" name="campo_ingreso_seguimientoDespacho_1" maxlength="10" style="width:100%" value="'.$rsql_maestro[0]['ans_nombre'].'" />';
            }else if($avance==0){
