...
<script>
    $(document).ready(function(){
        $("#campo_ingreso_seguimientoDespacho_1").change(function () {
            $("#campo_ingreso_seguimientoDespacho_1 option:selected").each(function () {
                var selanio=$(this).val();
                var seleje=$("#campo_ingreso_seguimientoDespacho_1").val();
                $.post("./class/comboObjgen.php", { async:false, anio: selanio,eje: seleje }, function(data){
                    $("#campo_ingreso_seguimientoDespacho_3").html(data);
                });
            });
        });
        $("#campo_ingreso_seguimientoDespacho_2").change(function () {
            $("#campo_ingreso_seguimientoDespacho_2 option:selected").each(function () {
                var selanio=$("#campo_ingreso_seguimientoDespacho_1").val();
                var seleje=$(this).val();
                $.post("./class/comboObjgen.php", { async:false, anio: selanio,eje: seleje }, function(data){
                    $("#campo_ingreso_seguimientoDespacho_3").html(data);
                });
...
if(isset($_REQUEST["_id_registro_seguimiento"]) && $_REQUEST["_id_registro_seguimiento"]!=NULL){
    $avance=1;
    $rsql_maestro=$_consulta_sistema->query( "select d.oes_codigo,a.ans_nombre,b.ejs_nombre,c.ogs_nombre,d.oes_nombre from
          ...
          where a.ans_codigo=c.ans_codigo and b.ejs_codigo=c.ejs_codigo and c.ogs_codigo=d.ogs_codigo and d.oes_codigo='{$_REQUEST['_id_registro_seguimiento']}' limit 1;" );
...
<div title="Ficha General" style="padding:10px; background:rgba(187, 187, 187, 0.0170588)">
    <form  name="frm_insertarobjesp_seguimiento_despacho" id="frm_insertarobjesp_seguimiento_despacho" style="width:100%; height:auto; border:1px solid #CCC">
    <table align="center" id="tableFichaGeneral_seguimiento_despacho" width="99%" cellpadding="5" cellspacing="5">
    <tbody>
    <tr>
        <td colspan="6" style="background:#0072C6; color:#fff">INGRESO OBJETIVOS ESPECÍFICOS</td>
    </tr>
            ...
            if ( in_array(40, $perfil,true) || in_array(41, $perfil,true)  ) :
                ...
                <script>
                    function actualizarObjgen_seguimientoDespacho(){
                        $.messager.confirm('Confirmación','Está seguro que desea actualizar el objetivo específico?.',function(r){
                            if (r){
                                $.ajax( "<?php echo DIR_REL ?>class/validador.php")
                                    .done(function(result) {
                                        if(result==1){
                                            xajax_procesar_frm_insertarobjesp_seguimiento_despacho(xajax.getFormValues('frm_insertarobjesp_seguimiento_despacho', true));
                                        }else if(result==0){
                                            location.reload();
                                        }
                                    });
    ...
    <tr>
        ...
        <td>
            ...
            <select id="campo_ingreso_seguimientoDespacho_1" name="campo_ingreso_seguimientoDespacho_1" style="width:100%">
                ...
                if($avance==1){
                    $auxfilas=0;
                    $sql=$_consulta_sistema->query("select a.ans_codigo,a.ans_nombre from seguimiento_despacho_entrega.ssd_anios_seguimmiento a,
                            seguimiento_despacho_entrega.ssd_objetivosgen_seguimmiento b,seguimiento_despacho_entrega.ssd_objetivosesp_seguimmiento c
                            where a.ans_codigo=b.ans_codigo and b.ogs_codigo=c.ogs_codigo and c.oes_codigo=".$_REQUEST["_id_registro_seguimiento"]);
    ...
    <tr>
        ...
        <td>
            ...
            <select id="campo_ingreso_seguimientoDespacho_2" name="campo_ingreso_seguimientoDespacho_2" style="width:100%">
                ...
                if($avance==1){
                    $auxfilas=0;
                    $sql=$_consulta_sistema->query("select a.ejs_codigo,a.ejs_nombre from seguimiento_despacho_entrega.ssd_ejes_seguimmiento a ,
                            seguimiento_despacho_entrega.ssd_objetivosgen_seguimmiento b,seguimiento_despacho_entrega.ssd_objetivosesp_seguimmiento c
                            where a.ejs_codigo=b.ejs_codigo and b.ogs_codigo=c.ogs_codigo and c.oes_codigo=".$_REQUEST["_id_registro_seguimiento"]);
                    for($i=0;$i<count($sql);$i++){
    ...
    <tr>
        ...
        <td>
            ...
            <select id="campo_ingreso_seguimientoDespacho_3" name="campo_ingreso_seguimientoDespacho_3" style="width:100%">
                <?php
                if($avance==1){
                    $auxfilas=0;
                    $sql=$_consulta_sistema->query("select a.ogs_codigo,a.ogs_nombre from seguimiento_despacho_entrega.ssd_objetivosgen_seguimmiento a,
                    seguimiento_despacho_entrega.ssd_objetivosesp_seguimmiento b where a.ogs_codigo=b.ogs_codigo and b.oes_codigo=".$_REQUEST["_id_registro_seguimiento"]);
                    for($i=0;$i<count($sql);$i++){

                        $sql2=$_consulta_sistema->query("select ogs_codigo,ogs_nombre from seguimiento_despacho_entrega.ssd_objetivosgen_seguimmiento order by ogs_codigo");
                        for($j=0;$j<count($sql2);$j++){
                            $auxfilas=$auxfilas+1;
                            if($sql[$i][0]==$sql2[$j][0]){
                                echo '<option value="'.$sql2[$j][0].'" selected>'.$sql2[$j][1].'</option>';
                  ...
                  /*  $sql=$_consulta_sistema->query(" select ogs_codigo,ogs_nombre from seguimiento_despacho_entrega.ssd_objetivosgen_seguimmiento order by ogs_codigo");
                    echo '<option value="0" selected>SELECCIONE UN OBJETIVO GENERAL</option>';
                    for($i=0;$i<count($sql);$i++){
                        echo '<option value="'.$sql[$i][0].'">'.$sql[$i][1].'</option>';
                    }*/
                    echo '<option value="0" selected>SELECCIONE UN AÑO Y EJE</option>';
                }
                ?>
            </select>
        </td>
    ...
    <tr>
        <td width="10%"><b>OBJETIVO ESPECÍFICO</b></td>
        <td width="70%">
            <?php
            /*if($avance==1){
            ...
            if($avance==1){
                echo '<textarea id="campo_ingreso_seguimientoDespacho_4"name="campo_ingreso_seguimientoDespacho_4" style="width:100%; resize:none" rows="6" >'.$rsql_maestro[0]['oes_nombre'].'</textarea>';
            }else if($avance==0){
                echo '<textarea id="campo_ingreso_seguimientoDespacho_4" name="campo_ingreso_seguimientoDespacho_4" style="width:100%; resize:none" rows="6" /></textarea>';
            }
            ?>
        </td>
        <td width="10%"><b></b></td>
