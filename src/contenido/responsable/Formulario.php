  	...
  	<div title="Ficha General" style="padding:10px; background:rgba(187, 187, 187, 0.0170588)">
  		<form  name="frm_fichageneral_seguimientoDespacho" id="frm_fichageneral_seguimientoDespacho" style="width:100%; height:auto; border:1px solid #CCC">
  						...
  						<td width="15%">
  							...
  							<script>
  								function actualizarGeneral_seguimientoDespacho(){
  									$.messager.confirm('ConfirmaciÃ³n','EstÃ¡ seguro que desea actualizar la ficha?.',function(r){
  										if (r){
            ...
            <tr>
  						...
  						<td colspan="5">
  							<select name="campo_ingreso_seguimientoDespacho_16" style="width:100%">
  								<?php
  									if($avance==1){
  										$sql=$_consulta_sistema->query("select obg.ogs_nombre,obg.ogs_nombre from seguimiento_despacho_entrega.ssd_objetivosgen_seguimmiento obg,seguimiento_despacho_entrega.ssd_objetivosesp_seguimmiento obe where obe.ogs_codigo = '{$rsql_maestro[0]['obg.ogs_codigo']}'");
  										for($i=0;$i<count($sql);$i++){
  											echo '<option value="'.$sql[$i][0].'">'.$sql[$i][1].'</option>';
  										}
            ...
            <tr>
  						...
  						<td colspan="5">
  							<select name="campo_ingreso_seguimientoDespacho_15" style="width:100%">
  								<?php
  									if($avance==1){
  										$sql=$_consulta_sistema->query("select oes_codigo,oes_nombre from seguimiento_despacho_entrega.ssd_objetivosesp_seguimmiento  where oes_codigo='{$rsql_maestro[0]['oes_codigo']}'");
  										for($i=0;$i<count($sql);$i++){
  											echo '<option value="'.$sql[$i][0].'">'.$sql[$i][1].'</option>';
  										}
  										$sql=$_consulta_sistema->query("select oes_codigo,oes_nombre from seguimiento_despacho_entrega.ssd_objetivosesp_seguimmiento  where oes_codigo!='{$rsql_maestro[0]['oes_codigo']}' order by oes_nombre");
  						...
  						<td colspan="2">
  							...
  							<script>
  								$(function() {
  									...
  									placeholder: "ESCOJA UNA OPCIÃ“N",allowClear: true});
  								});
  							</script>
  						</td>
  					</tr>
  					<tr>
  						...
  						<td colspan="2">
  							<?php
  								if($avance==1){
  									echo '<input id="campo_ingreso_seguimientoDespacho_8" name="campo_ingreso_seguimientoDespacho_8" style="width:100%" value="'.$rsql_maestro[0]['rse_fecha_inicio'].'" />';
  									}else if($avance==0){
  	...
  	<div title="Antecedentes" data-options="iconCls:'',closable:false" style="padding:10px; background:rgba(187, 187, 187, 0.0170588)">
  		<form name="frm_fichaantecedentes_seguimientoDespacho" id="frm_fichaantecedentes_seguimientoDespacho" style="width:100%; height:auto; border:1px solid #CCC">
              <table id="tableFichaAntecedentes_seguimiento_despacho" width="99%" cellpadding="5" cellspacing="5">
  				<tbody>
  					<tr>
  						<td colspan="2" style="background:#0072C6; color:#fff">INFORMACIÃ“N</td>
  					</tr>
  					<tr style="display:none; visibility:hidden">
  						<td colspan="2">
  							<?php
  					...
  					<tr>
  						...
  						<td valign="top" ><input id="campo_antecedentes_seguimientoDespacho_2" name="campo_antecedentes_seguimientoDespacho_2" value="<?php echo date("Y-m-d") ?>" style="width:40%" />
  							<?php
  								if ( in_array(40, $perfil,true) || in_array(41, $perfil,true)  ) :
  							?>
  							<a onclick="actualizarAntecedentes_seguimientoDespacho()" class="easyui-linkbutton" data-options="iconCls:'icon-cloud'" style="width:100px">AÃ±adir</a>
  							<script>
  		...
  		<form name="frm_fichaparticipantes_seguimientoDespacho" id="frm_fichaparticipantes_seguimientoDespacho" style="width:100%; height:auto; border:1px solid #CCC">
  				...
  				<tbody>
  					...
  					<tr>
  						...
  						<td>
  							...
  							<script>
  								$(function() {
  									$("#campo_participantes_seguimientoDespacho_1").select2({  placeholder: "ESCOJA UNA OPCIÃ“N",allowClear: true});
  								});
  							</script>
  						</td>
  					</tr>
  					<tr>
  						...
  						<td>
  							...
  							<?php
  								if ( in_array(40, $perfil,true) || in_array(41, $perfil,true)  ) :
  							?>
  							<a onclick="actualizarParticipantes_seguimientoDespacho()" class="easyui-linkbutton" data-options="iconCls:'icon-cloud'" style="width:100px">AÃ±adir</a>
  							<script>
  						...
  						<td colspan="2">
  							...
  							<script type="text/javascript">
  								function eliminaResp(){
  									$.messager.confirm('ConfirmaciÃ³n','EstÃ¡ seguro que desea eliminar al responsable?.',function(r){
  										if (r){
  			...
  			<script type="text/javascript">
  				function eliminacoResp(id){
  					$.messager.confirm('ConfirmaciÃ³n','EstÃ¡ seguro que desea eliminar al coresponsable?.',function(r){
  						if (r){
  				...
  				function eliminaPart(id){
  					$.messager.confirm('ConfirmaciÃ³n','EstÃ¡ seguro que desea eliminar al participantes?.',function(r){
  						if (r){
  									...
  									for($k=0;$k<count($rsql);$k++){
  										...
  										if($rsql[$k][1]!=NULL){
  											if($rsql[$k][5]==2){
  												...
  												echo "<b style=\"color:green\">APROBADO POR: </b> ".$rsqlpersona0[0][0]." <b>EL DÃA </b> ".FechaFormateada2(strtotime($rsql[$k][2])).' '.date('H:i',strtotime($rsql[$k][2]))."<br />";
  											}
  											if($rsql[$k][5]==3){
  												if ( in_array(40, $perfil,true) || in_array(41, $perfil,true)  ) :
  												echo '<img onclick="revision_avance_seguimiento_despacho('.$rsql[$k][6].',1)" src="'.DIR_LOCAL_IMG_SEGUIMIENTO_DESPACHO.'volver_16.png"><br />';
  												endif;
  												echo "<b style=\"color:red\">RECHAZADO POR: </b> ".$rsqlpersona0[0][0]." <b>EL DÃA </b> ".FechaFormateada2(strtotime($rsql[$k][2])).' '.date('H:i',strtotime($rsql[$k][2]))."<br />";
  											}
  										}
  										echo "<b>% DE AVANCE:</b> ".$rsql[$k][7]."<br />";
  										echo "<b>ESCRITO POR: </b> ".$rsqlpersona1[0][0]." <b>EL DÃA </b> ".FechaFormateada2(strtotime($rsql[$k][0])).' '.date('H:i',strtotime($rsql[$k][0]))." <br><b>AVANCE: </b>".$rsql[$k][4]." <br/><hr color=\"#e2e2e2\">";
  									}
  ...
  <?php
  	...
  	$.messager.confirm(\'ConfirmaciÃ³n\',\'EstÃ¡ seguro que desea actualizar este avance?.\',function(r){
  	if (r){
  							...
  							if($avance==1){
  								...
  								if( count($rsqlmensaje[0][0]>0) ){
  									...
  									for($k=0;$k<count($rsql);$k++){
  										...
  										echo "<b>ESCRITO POR: </b> ".$rsqlpersona1[0][0]." <b>EL DÃA </b> ".FechaFormateada2(strtotime($rsql[$k][0])).' '.date('H:i',strtotime($rsql[$k][0]))." <br><b>MENSAJE: </b>".$rsql[$k][4]."<br /><hr color=\"#e2e2e2\">";
  									}
  									echo '</div>';
  									}else{
  									echo '<div style="width:100%;"></div>';
  ...
  <div title="Archivos" data-options="iconCls:'',closable:false" style="padding:10px; background:rgba(187, 187, 187, 0.0170588)">
              ...
              <tbody>
  				...
  				<script type="text/javascript">
  					...
  					uploader.bind('UploadProgress', function(up, file) {
  						Â Â document.getElementById(file.id).getElementsByTagName('b')[0].innerHTML = '<span>' + file.percent + "%</span>";
  					});
  					Â 
  									...
  									for($k=0;$k<count($rsql);$k++){
  										...
  										if($rsql[$k][3]!=NULL){
  											if($rsql[$k][7]==10){
  												...
  												echo "<b style=\"color:green\">APROBADO POR: </b> ".$rsqlpersona1[0][0]." <b>EL DÃA </b> ".FechaFormateada2(strtotime($rsql[$k][4])).' '.date('H:i',strtotime($rsql[$k][4]))."<br />";
  											}
  											if($rsql[$k][7]==11){
  												if ( in_array(40, $perfil,true) || in_array(41, $perfil,true)  ) :
  												echo '<img onclick="revision_archivo_seguimiento_despacho('.$rsql[$k][8].',9)" src="'.DIR_LOCAL_IMG_SEGUIMIENTO_DESPACHO.'volver_16.png"><br />';
  												endif;
  												echo "<b style=\"color:red\">RECHAZADO POR: </b> ".$rsqlpersona1[0][0]." <b>EL DÃA </b> ".FechaFormateada2(strtotime($rsql[$k][4])).' '.date('H:i',strtotime($rsql[$k][4]))."<br />";
  											}
  										}



  										echo "<b>CARGADO POR: </b> ".$rsqlpersona0[0][0]." <b>EL DÃA </b> ".FechaFormateada2(strtotime($rsql[$k][2])).' '.date('H:i',strtotime($rsql[$k][2]))."<br>";
  										echo "<b>NOMBRE DEL ARCHIVO: </b> ".$rsql[$k][5]." <br />";
  										echo "<b>VISUALIZAR ARCHIVO: </b><a style='color:#000' href='".$rsql[$k][6]."' target=\"_blank\">Presione  AquÃ­</a><br><hr color=\"#e2e2e2\">";
  									}
  									echo '</div>';
  									}else if($avance==0){
  	...
  	<div title="HistÃ³rico" data-options="iconCls:'',closable:false" style="padding:10px; background:rgba(187, 187, 187, 0.0170588)">
  		<form  style="width:100%; height:98%; border:1px solid #CCC">
  			<table id="tableFichaHistorico_seguimiento_despacho" width="99%" cellpadding="5" cellspacing="5">
  				<tbody>
  					<tr>
  						<td colspan="3" style="background:#0072C6; color:#fff">INFORMACIÃ“N</td>
  					</tr>
  					<tr>
  							...
  							<?php
  								if($avance==1){
  									...
  									for($k=0;$k<count($rsql);$k++){
  										...
  										echo "<b>CAMBIADO POR: </b> ".$rsqlpersona0[0][0]." <b>EL DÃA </b>".FechaFormateada2(strtotime($rsql[$k][1])).' '.date('H:i',strtotime($rsql[$k][1]))."<br /><hr color=\"#e2e2e2\">";
  									}
  									echo '</div>';
  									}else if($avance==0){
  									echo 'div style="width:100%" ></div>';
  							...
  							<?php
  								if($avance==1){
  									...
  									for($k=0;$k<count($rsql);$k++){
  										...
  										echo "<b>CAMBIADO POR: </b> ".$rsqlpersona0[0][0]." <b>EL DÃA </b>".FechaFormateada2(strtotime($rsql[$k][0])).' '.date('H:i',strtotime($rsql[$k][0]))."<br /><hr color=\"#e2e2e2\">";
  									}
  									echo '</div>';
  									}else if($avance==0){
  									echo 'div style="width:100%" ></div>';
  						...
  						<td  valign="top" width="33%">
  							<?php
  								if($avance==1){
  									...
  									for($k=0;$k<count($rsql);$k++){
  										$rsqlpersona0=$_consulta_login->query(" select per_nombres from aplicativo_web.vw_informacion_persona where  per_codigo='{$rsql[$k][1]}' ");
  										echo "<b>VISITANTE:</b> ".$rsqlpersona0[0][0]."<br/><b>DÃA: </b>".FechaFormateada2(strtotime($rsql[$k][0])).' '.date('H:i',strtotime($rsql[$k][0]))."<br /><hr color=\"#e2e2e2\">";
  									}
  									echo '</div>';
  									}else if($avance==0){
  									echo 'div style="width:100%" ></div>';
