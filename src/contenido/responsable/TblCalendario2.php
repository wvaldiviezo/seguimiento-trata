...
echo "<table class=\"calendario_seguimiento_despacho\" rules='all' cellpadding='2' cellspacing='0' style='width:100%; height:100%;  padding 10px;' align='center'>";
  ...
  where  a.rse_fecha_fin='".date('Y-m-d', strtotime($primerDia) + $inicial)."' and (a.ese_codigo!=7 and a.ese_codigo!=8 ) {$where} order by a.rse_codigo ");
        $sql2=$_consulta_sistema->query("SELECT a.rse_codigo,a.rse_nombre,d.ase_nombre
  ...
  where  a.rse_fecha_fin='".date('Y-m-d', strtotime($primerDia) + $inicial)."' and (a.ese_codigo!=7 and a.ese_codigo!=8 ) {$where2} order by a.rse_codigo ");
        $sql3=$_consulta_sistema->query("SELECT a.rse_codigo,a.rse_nombre,d.ase_nombre
  FROM seguimiento_despacho_entrega.ssd_registro_seguimiento a
  left join seguimiento_despacho_entrega.ssd_avance_seguimiento d on a.ase_codigo=d.ase_codigo
  left join seguimiento_despacho_entrega.ssd_participante_seguimiento b on a.rse_codigo=b.rse_codigo
  where  a.rse_fecha_fin='".date('Y-m-d', strtotime($primerDia) + $inicial)."' and (a.ese_codigo!=7 and a.ese_codigo!=8 ) {$where3} order by a.rse_codigo ");
        if(count($sql)>0){
			echo "<td valign='top'>";
            ...
            for($j=0;$j<count($sql2);$j++){
                echo "<tr><td style='background-color:#0072C6; color: #fff' onclick='abrirFormulario_busqueda(".$sql2[$j][0].")'>".$sql2[$j][0]."</td></tr>";
            }
            echo "</table>";
            echo "</td>";
        }elseif(count($sql3)>0){
            echo "<td valign='top'>";
            echo "<table style='width:100%;' cellpadding='3' cellspacing='0' >";
            for($j=0;$j<count($sql3);$j++){
                echo "<tr><td style='background-color:#0072C6; color: #fff' onclick='abrirFormulario_busqueda(".$sql3[$j][0].")'>".$sql3[$j][0]."</td></tr>";
            }
            echo "</table>";
            echo "</td>";
		}else{
