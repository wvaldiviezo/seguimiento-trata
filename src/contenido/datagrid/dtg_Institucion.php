...
if(isset($_SESSION['logueado'])){
				...
				if ($op == 'contains'){
					switch($field){
						case "cmp0VisorBusqueda_seguimiento_despacho":
							$where .= " and a.ins_codigo = '$value' ";
						break;
						case "cmp1VisorBusqueda_seguimiento_despacho":
							$where .= " and a.ins_nombre like '%$value%' ";
						break;
					}
				}
			}
	...
	$rs1 = $_consulta_sistema->query("SELECT count(a.*) FROM seguimiento_despacho_entrega.ssd_instituciones_seguimiento a where a.ins_codigo=a.ins_codigo  {$where} ");
	$json="";
	$json.= '{"total":"'.$rs1[0][0].'","rows":[ ';
	$rs = $_consulta_sistema->query(" SELECT a.ins_codigo,a.ins_nombre
  FROM seguimiento_despacho_entrega.ssd_instituciones_seguimiento a
  where  a.ins_codigo=a.ins_codigo {$where} order by a.ins_codigo asc
