...
if(isset($_SESSION['logueado'])){
				...
				if ($op == 'contains'){
					switch($field){
						case "cmp0VisorBusqueda_seguimiento_despacho":
							$where .= " and a.ejs_codigo = '$value' ";
						break;
						case "cmp1VisorBusqueda_seguimiento_despacho":
							$where .= " and a.ejs_nombre=$value ";
						break;
					}
				}
			}
	...
	$rs1 = $_consulta_sistema->query("SELECT count(a.*) FROM seguimiento_despacho_entrega.ssd_ejes_seguimmiento a where a.ejs_codigo=a.ejs_codigo  {$where} ");
	$json="";
	$json.= '{"total":"'.$rs1[0][0].'","rows":[ ';
	$rs = $_consulta_sistema->query(" SELECT a.ejs_codigo,a.ejs_nombre
  FROM seguimiento_despacho_entrega.ssd_ejes_seguimmiento a
  where  a.ejs_codigo=a.ejs_codigo {$where} order by a.ejs_codigo asc
