...
if(isset($_SESSION['logueado'])){
	...
	$rs1 = $_consulta_sistema->query("select count(c.*) from seguimiento_despacho_entrega.ssd_anios_seguimmiento a,
            seguimiento_despacho_entrega.ssd_ejes_seguimmiento b,seguimiento_despacho_entrega.ssd_objetivosgen_seguimmiento c
            where a.ans_codigo=c.ans_codigo and b.ejs_codigo=c.ejs_codigo  {$where} ");
	$json="";
	$json.= '{"total":"'.$rs1[0][0].'","rows":[ ';
	$rs = $_consulta_sistema->query(" select c.ogs_codigo,a.ans_nombre,b.ejs_nombre,c.ogs_nombre
            ...
            and b.ejs_codigo=c.ejs_codigo {$where} order by a.ans_nombre,b.ejs_nombre,c.ogs_codigo asc
  limit $rows offset $offset");

	for($i=0;$i<count($rs);$i++){
		
		$json.='{"cmp0VisorBusqueda_seguimiento_despacho":"'.$rs[$i][0].'","cmp1VisorBusqueda_seguimiento_despacho":"'.$rs[$i][1].'","cmp2VisorBusqueda_seguimiento_despacho":"'.$rs[$i][2].'","cmp3VisorBusqueda_seguimiento_despacho":"'.$rs[$i][3].'"}';
		if($i<count($rs)-1){
			$json.=",";
		}
	}
