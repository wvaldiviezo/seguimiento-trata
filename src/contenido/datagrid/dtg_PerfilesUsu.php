...
if(isset($_SESSION['logueado'])){
				...
				if ($op == 'contains'){
					switch($field){
                        ...
                        case "cmp3VisorBusqueda_seguimiento_despacho":
                            $where .= " and translate (upper(b.ins_nombre), 'áéíóúÁÉÍÓÚäëïöüÄËÏÖÜñ', 'aeiouAEIOUaeiouAEIOUÑ') like translate (upper('%$value%'), 'áéíóúÁÉÍÓÚäëïöüÄËÏÖÜñ', 'aeiouAEIOUaeiouAEIOUÑ') ";
                        break;
                        case "cmp4VisorBusqueda_seguimiento_despacho":
                            $where .= " and translate (upper(a.per_mail), 'áéíóúÁÉÍÓÚäëïöüÄËÏÖÜñ', 'aeiouAEIOUaeiouAEIOUÑ') like translate (upper('%$value%'), 'áéíóúÁÉÍÓÚäëïöüÄËÏÖÜñ', 'aeiouAEIOUaeiouAEIOUÑ') ";
                        break;
	...
	$rs1 = $_consulta_sistema->query("SELECT count(a.*) FROM aplicativo_web.aaw_persona a where a.per_codigo=a.per_codigo  {$where} ");
	$json="";
	$json.= '{"total":"'.$rs1[0][0].'","rows":[ ';
	$rs = $_consulta_sistema->query(" SELECT a.per_codigo,a.per_apellidos,a.per_nombres,a.per_cargo_laboral,a.per_mail,a.per_telefono,a.per_extension,a.per_celular,b.ins_nombre
          FROM aplicativo_web.aaw_persona a
          left join seguimiento_despacho_entrega.ssd_instituciones_seguimiento b on a.ins_codigo=b.ins_codigo
          where  a.per_codigo=a.per_codigo {$where} order by a.per_apellidos,a.per_nombres  desc
          limit $rows offset $offset");
	for($i=0;$i<count($rs);$i++){
		
		$json.='{"cmp0VisorBusqueda_seguimiento_despacho":"'.$rs[$i][0].'","cmp1VisorBusqueda_seguimiento_despacho":"'.$rs[$i][1].'","cmp2VisorBusqueda_seguimiento_despacho":"'.$rs[$i][2].'","cmp3VisorBusqueda_seguimiento_despacho":"'.$rs[$i][8].'","cmp4VisorBusqueda_seguimiento_despacho":"'.$rs[$i][4].'","cmp5VisorBusqueda_seguimiento_despacho":"'.$rs[$i][5].'","cmp6VisorBusqueda_seguimiento_despacho":"'.$rs[$i][6].'","cmp7VisorBusqueda_seguimiento_despacho":"'.$rs[$i][7].'"}';
		if($i<count($rs)-1){
			$json.=",";
		}
	}
