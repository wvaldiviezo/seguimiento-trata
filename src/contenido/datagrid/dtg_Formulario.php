<?php
	...
	if (!empty($filterRules)){
		...
		foreach($filterRules as $rule){
			...
			if (!empty($value)){
				if ($op == 'contains'){
					switch($field){
						case "cmp1Formulario_salida":
							$where .= " and cedula like '%$value%' ";
						break;
						case "cmp2Formulario_salida":
  ...
  FROM salida_personal.ssp_codigo_salida where eho_codigo!=4 limit $rows offset $offset");
	for($i=0;$i<count($rs);$i++){
		...
		//$json.='{"cmp0Formulario_salida":"'.urlencode($codigo).'","cmp1Formulario_salida":"'.$rs2[0][0].'","cmp2Formulario_salida":"'.$rs3[0][0].'","cmp3Formulario_salida":"'.FechaFormateada2(strtotime($rs[$i][3])).'","cmp4Formulario_salida":"'.$rs[$i][4].'","cmp5Formulario_salida":"'.$rs[$i][5].'","cmp6Formulario_salida":"'.$rs[$i][6].'","cmp7Formulario_salida":"'.$rs[$i][7].'","cmp8Formulario_salida":"'.$rs[$i][8].'"}';
		$json.='{"cmp0Formulario_salida":"'.$rs[$i][0].'","cmp1Formulario_salida":"'.$rs2[0][0].'","cmp2Formulario_salida":"'.$rs3[0][0].'","cmp3Formulario_salida":"'.interval_date($rs[$i][3])." - ".FechaFormateada2(strtotime($rs[$i][3])).'"}';		
		if($i<count($rs)-1){
			$json.=",";
		}
	}
